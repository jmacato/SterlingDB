...
namespace SterlingDB.Keys
{
    ...
    internal class KeyCollection<T, TKey> : IKeyCollection where T : class, new()
    {
        ...
        /// </summary>
        public List<TableKey<T, TKey>> Query => new List<TableKey<T, TKey>>(_keyList);
        ...
        public async Task FlushAsync()
        {
            ...
            {
                if (IsDirty) await _SerializeKeysAsync().ConfigureAwait(false);
        ...
        public async Task<int> GetIndexForKeyAsync(object key)
        {
            using (await _lock.LockAsync().ConfigureAwait(false))
            {
                ...
                if (_keyMap.TryGetValue((TKey) key, out result) == false) result = -1;
        ...
        private async Task _DeserializeKeysAsync()
        {
            ...
            if (keyMap == null) keyMap = new Dictionary<TKey, int>();
