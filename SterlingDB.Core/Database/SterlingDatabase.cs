...
namespace SterlingDB.Database
{
    ...
    internal class SterlingDatabase : ISterlingDatabase
    {
        ...
        /// </summary>
        private readonly Dictionary<string, Tuple<Type, ISterlingDatabaseInstance>> _databases =
            new Dictionary<string, Tuple<Type, ISterlingDatabaseInstance>>();
        ...
        public SterlingEngine Engine { get; }
        ...
        public async Task BackupAsync<T>(BinaryWriter writer) where T : BaseDatabaseInstance
        {
            ...
            if (!databaseQuery.Any()) throw new SterlingDatabaseNotFoundException(typeof(T).FullName);
            ...
            foreach (var type in typeMaster) writer.Write(type);

            // now iterate tables
            foreach (var table in ((BaseDatabaseInstance) database).TableDefinitions)
            {
                // get the key list
                var keys = await database.Driver
                    .DeserializeKeysAsync(table.Key, table.Value.KeyType, table.Value.GetNewDictionary())
        ...
        public async Task RestoreAsync<T>(BinaryReader reader) where T : BaseDatabaseInstance
        {
            ...
            if (!version.Equals(_databaseVersion)) throw new SterlingException("Unexpected database version.");
            ...
            foreach (var table in ((BaseDatabaseInstance) database).TableDefinitions)
            {
                ...
                foreach (var index in table.Value.Indexes) await index.Value.FlushAsync().ConfigureAwait(false);
            }
        }
        ...
        public ISterlingDatabaseInstance RegisterDatabase<T, TDriver>(string instanceName)
            where T : BaseDatabaseInstance where TDriver : ISterlingDriver
        {
        ...
        public ISterlingDatabaseInstance GetDatabase(string databaseName)
        {
            ...
            if (!_databases.ContainsKey(databaseName)) throw new SterlingDatabaseNotFoundException(databaseName);
        ...
        {
            if (!_activated) throw new SterlingNotReadyException();
        }
    }
}