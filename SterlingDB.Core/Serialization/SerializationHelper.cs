...
namespace SterlingDB.Serialization
{
    ...
    public class SerializationHelper
    {
        ...
        /// <summary>
        ///     The import cache, stores what properties are available and how to access them. Each type has a matching dictionary
        ...
        private ISterlingPlatformAdapter PlatformAdapter => _databaseInstance.Database.Engine.PlatformAdapter;
            ...
            lock (((ICollection) _propertyCache).SyncRoot)
            {
                ...
                var fields = from f in PlatformAdapter.GetFields(type)
                    where
                        !f.IsStatic &&
                        !f.IsLiteral &&
                        !f.IsIgnored(_databaseInstance.IgnoreAttribute) &&
                        !f.FieldType.IsIgnored(_databaseInstance.IgnoreAttribute, PlatformAdapter)
                ...
                var properties = from p in PlatformAdapter.GetProperties(type)
                    where
                        ...
                        && !p.IsIgnored(_databaseInstance.IgnoreAttribute) &&
                        !p.PropertyType.IsIgnored(_databaseInstance.IgnoreAttribute, PlatformAdapter)
                ...
                foreach (var p in properties.Concat(fields))
                {
                    ...
                    _propertyCache[type].Add(p1.Name,
                        new SerializationCache(propType, p1.Name, (parent, property) => p1.Setter(parent, property),
                            p1.GetValue));
                }
            }
        }
        ...
        public void Save(Type type, object instance, BinaryWriter bw, CycleCache cache, bool saveTypeExplicit)
        {
            ...
            else if (saveTypeExplicit) bw.Write(_typeResolver(type.AssemblyQualifiedName));
            ...
            foreach (var p in new Dictionary<string, SerializationCache>(_propertyCache[type]))
            {
                ...
                _InnerSave(value == null ? serializationCache.PropType : value.GetType(),
                    serializationCache.PropertyName, value, bw, cache);
            }
        ...
        private void _SaveDictionary(IDictionary dictionary, BinaryWriter bw, CycleCache cache)
        {
            ...
            foreach (var item in dictionary.Keys)
            {
                ...
                _InnerSave(dictionary[item] == null ? typeof(string) : dictionary[item].GetType(),
                    "DictionaryItemValue", dictionary[item], bw, cache);
            }
        }
        ...
        /// <param name="bw">The writer</param>
        /// <param name="cache">Cycle cache</param>
        private void _SerializeClass(Type type, string propertyName, object foreignTable, BinaryWriter bw,
            CycleCache cache)
        {
            bw.Write(propertyName + PROPERTY_VALUE_SEPARATOR);
            bw.Write(_typeResolver(type.AssemblyQualifiedName));
            ...
            var task = _databaseInstance._Save<object>(foreignTable.GetType(), foreignTable.GetType(), foreignTable,
                cache);
        ...
        public T Load<T>(BinaryReader br)
        {
            ...
            if (node != null) return node.UnwrapForDeserialization<T>();
            return default;
        }
        ...
        public object Load(Type type, object key, BinaryReader br, CycleCache cache)
        {
            ...
            if (_DeserializeNull(br)) return null;
            ...
            if (instance is Array)
            {
                ...
                if (!isNull)
                {
                    ...
                    for (var x = 0; x < count; x++) ((Array) instance).SetValue(_Deserialize(br, cache).Value, x);
                }
            }
            else if (instance is IList)
            {
                ...
                cache.Add(type, instance, key);
                var isNull = _DeserializeNull(br);
                if (!isNull) _LoadList(br, cache, instance as IList);
            }
            ...
            else if (instance is IDictionary)
            {
                ...
                cache.Add(type, instance, key);
                var isNull = _DeserializeNull(br);
                if (!isNull) _LoadDictionary(br, cache, instance as IDictionary);
            }
            else
            {
                type = Type.GetType(_typeIndexer(br.ReadInt32()));
                if (instance.GetType() != type) instance = Activator.CreateInstance(type);
        ...
        /// <summary>
        ///     Deserializes the next part in the BinaryReader and returns a KeyValuePair containing the property name as key van
        ///     the deserialized object as value.
        /// </summary>
        ...
        private KeyValuePair<string, object> _Deserialize(BinaryReader br, CycleCache cache)
        {
            ...
            if (_DeserializeNull(br)) return new KeyValuePair<string, object>(propertyName, null);
            ...
            if (!_typeRef.TryGetValue(typeName, out typeResolved))
            {
                ...
                {
                    if (!_typeRef.ContainsKey(typeName)) _typeRef.Add(typeName, typeResolved);
                }
            }
            ...
            if (_databaseInstance.IsRegistered(typeResolved))
            {
                ...
                if (cached != null) return new KeyValuePair<string, object>(propertyName, cached);
            ...
            if (PlatformAdapter.IsAssignableFrom(typeof(Array), typeResolved))
            {
                ...
                for (var x = 0; x < count; x++) array.SetValue(_Deserialize(br, cache).Value, x);
        ...
        private void _IteratePropertiesUntilEndOfFileIsReached(BinaryReader br, CycleCache cache, Type typeResolved,
            object instance)
        {
            var propertyPair = _Deserialize(br, cache);
        ...
        {
            var count = br.ReadInt32();
            for (var x = 0; x < count; x++) list.Add(_Deserialize(br, cache).Value);
            return list;
        }
        ...
        private void _SerializeNull(BinaryWriter bw, bool isNull)
        {
            ...
            _logManager.Log(SterlingLogLevel.Verbose, string.Format("{0}", isNull ? NULL_DISPLAY : NOTNULL_DISPLAY),
                null);
        }
