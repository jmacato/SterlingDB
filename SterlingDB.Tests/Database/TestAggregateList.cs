    ...
    public class TestAggregateList : TestBase
    {
        ...
        public void TestList()
        {
            ...
            for (var x = 0; x < expected.Children.Count; x++)
            {
                ...
                Assert.Equal(expected.Children[x].BaseProperty,
                    actual.Children[x].BaseProperty); //, "Save/load failed: data mismatch.");
                Assert.Equal(expected.Children[x].GetType(),
                    actual.Children[x].GetType()); //, "Save/load failed: type mismatch.");
            }
        }
