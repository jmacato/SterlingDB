...
namespace SterlingDB.Test.Database
{
    public class TestAsync : TestBase
    {
        public TestAsync()
        {
            ...
            for (var i = 0; i < MODELS; i++) _modelList.Add(TestModel.MakeTestModel());
        }
        ...
        public void TestConcurrentSaveAndLoad()
        {
            ...
            foreach (var item in _modelList) _databaseInstance.SaveAsync(item).Wait();
            ...
            var save = new Task(() =>
            {
                try
                {
                    ...
                    if (MODELS != savedCount) throw new Exception("Save failed: Not all models were saved.");
                }
                catch (Exception ex)
                {
            ...
            var load = new Task(() =>
            {
                try
                {
                    ...
                    if (list.Count < 1) throw new Exception("Test failed: did not load any items.");
                }
                catch (Exception ex)
                {
                    errorMsg = ex.AsExceptionString();
        ...
        public void TestConcurrentSaveAndLoadWithIndex()
        {
            ...
            var load = new Task(() =>
            {
                try
                {
                    ...
                    if (list.Count < 1) throw new Exception("Test failed: did not load any models.");
                }
                catch (Exception ex)
                {
                    errorMsg = ex.AsExceptionString();
